...
\begin{proof}
    ...
    При $x\not=x^0$ сумма $\sum\limits_{1 < k,l\leq n} a_{k,l} (x_k-x_k^0) (x_l-x_l^0) = \langle A(x-x^0), (x-x^0) \rangle > 0$
    $\Rightarrow \exists \varepsilon>0: \ \langle A(x-x^0), (x-x^0) \rangle \geq \varepsilon ||x-x^0||^2$

    \textit{Пояснение: } $\langle Ax,x \rangle > 0, x\not=0$, $A$ - линейное,
    пусть $g(x)=\langle Ax,x \rangle$\\
    $\Rightarrow \exists \varepsilon > 0: \ g(x) \geq \varepsilon$ при $||x||=1$ (единичная сфера), 
    т.к. функция $g(x)$ на единичной сфере (сфера - компакт) достигает минимума и максимума,
    также легко видеть, что $g(cx)=c^2 g(x)$, где $c$ - скаляр. 
    Тогда $g(x)=g(||x||e)=||x||^2 g(e) \geq \varepsilon ||x||^2$, т.к. $||e||=1$.

    Поделим равенство
    \[
        f(x) - f(x^0) = \langle A(x-x^0), (x-x^0) \rangle + o(||x-x_0||^2)
    \]
...
\begin{defn}
    $\mathcal{A}$ - {algebra}{Алгебра в пространстве непрерывных функций},
